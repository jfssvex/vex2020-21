...
void myAutonomous() {
    ...
    Vector2 secondBall(64, 44);
    turnToAngle(radToDeg((secondBall - trackingData.getPos()).getAngle()) - 90);
    in();
    strafeToPoint(secondBall);
    turnToAngle(-175);
    stopRollers();
    alignAndShoot(goalBC, radToDeg(trackingData.getHeading()), 2, false);
    stopRollers();
    strafeToPoint(secondBall);
    Vector2 thirdBall(95, 34);
    ...
    in();
    strafeToOrientation(Vector2(80, 70), 45);
    // strafeToPoint(Vector2(90.5, 67));
    turnToAngle(-95);
    strafeToPoint(Vector2(110, 80));
    stopRollers();
    alignAndShoot(goalCR, -80, 1);
    stopRollers();
    strafeRelative(Vector2(-20, 0));

    Vector2 topRightBall = thirdBall + Vector2(2, 85);
    in();
